...
void SquareComponent::Render() const
{
	...
	rectToRender.x = m_Rect.x + static_cast<int>(m_pParent->GetTransform()->GetPosition().x) + m_offset.x;
	rectToRender.y = m_Rect.y + static_cast<int>(m_pParent->GetTransform()->GetPosition().y) + m_offset.y;
